...
import { editReading, getReadingById } from '@/api/reading.api';
...
interface EditReadingFormValues {
  id: string;
  title: string;
  content: string;
  thumbnail: string;
...
export default function EditReadingForm() {
  ...
  const mutation = useMutation({
    mutationFn: (values: EditReadingFormValues) => editReading(values),
    onSuccess: (data) => {
  ...
  if (reading)
    return (
      <Box>
        <Formik
          initialValues={{ id: id, title: reading.title, thumbnail: '' }}
            ...
            <Form>
              <VStack alignItems='start' gap={4}>
                ...
                <FormikField name='title'>
                  {({ field, form }: { field: FieldInputProps<string>; form: FormikProps<EditReadingFormValues> }) => (
                ...
                <FormikField name={'thumbnail'}>
                  {({ form }: { form: FormikProps<EditReadingFormValues> }) => (
                    <Field
                      label='Thumbnail'
